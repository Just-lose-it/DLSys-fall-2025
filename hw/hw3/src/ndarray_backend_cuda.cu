...
__global__ void CompactKernel(const scalar_t* a, scalar_t* out, size_t size, CudaVec shape,
  ...
  if(gid>=size)return;
  int old_pos=offset;
  int n_dim=shape.size;
  int tmp=gid; 
  for(int j=n_dim-1;j>=0;j--){
    int idx=tmp%shape.data[j];
    tmp/=shape.data[j];
    old_pos+=idx*strides.data[j];
  }
  out[gid]=a[old_pos];
  /// BEGIN SOLUTION
...
const int V=4,L=32,S=16;
__global__ void mm(const scalar_t* a,const scalar_t* b,scalar_t* out,int m,int n,int p)
{
  ...
  for(int i=0;i<V;i++)
    for(int j=0;j<V;j++)
      C[i][j]=0;
  float A[V],B[V];
  int yblock=blockIdx.y,xblock=blockIdx.x;
  for(int ko=0;ko<n;ko+=S)
  {
    __syncthreads();
    //sa
    int load_start_x=ko+(V*S/L)*threadIdx.x,load_start_y=blockIdx.y*L+threadIdx.y*V;
    int loc_x=(V*S/L)*threadIdx.x,loc_y=threadIdx.y*V;
    for(int i=0;i<V;i++)
      for(int j=0;j<V*S/L;j++)
      {
        if(load_start_y+i<m&&load_start_x+j<n)
        {
          sA[loc_y+i][loc_x+j]=a[(load_start_y+i)*n+load_start_x+j];
        }
        else
        {
          sA[loc_y+i][loc_x+j]=0;
        }
      }
    //sb
    load_start_x=blockIdx.x*L+threadIdx.x*V,load_start_y=ko+(V*S/L)*threadIdx.y;
    loc_x=threadIdx.x*V,loc_y=(V*S/L)*threadIdx.y;
    
    for(int j=0;j<V*S/L;j++)
      for(int i=0;i<V;i++)
      {
        if(load_start_y+j<n&&load_start_x+i<p)
        {
          sB[loc_y+j][loc_x+i]=b[(load_start_y+j)*p+load_start_x+i];
        }
        else
        {
          sB[loc_y+j][loc_x+i]=0;
        }
      }
    __syncthreads();
     loc_x=threadIdx.x*V,loc_y=V*threadIdx.y;
  ...
  int store_start_x=blockIdx.x*L+threadIdx.x*V,store_start_y=blockIdx.y*L+threadIdx.y*V;
  for(int i=0;i<V;i++)
    for(int j=0;j<V;j++)
    {
      if(store_start_x+j<p && store_start_y+i<m)
      {
        out[(store_start_y+i)*p+store_start_x+j]=C[i][j];
      }
    }
}
...
void Matmul(const CudaArray& a, const CudaArray& b, CudaArray* out, uint32_t M, uint32_t N,
  ...
  CudaDims dim;
  int block_dim=L/V;
  size_t num_blocks_x = (P + L - 1) / L;
  size_t num_blocks_y = (M + L - 1) / L;
  dim.block = dim3(block_dim,block_dim , 1);
