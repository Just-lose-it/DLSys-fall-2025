...
class NDArray:
    ...
    def reshape(self, new_shape):
        ...
        if prod(self.shape)!=prod(new_shape):
          raise ValueError("current shape not equal to new shape")
        stride = 1
        res = []
        ...
        return self.as_strided(new_shape,tuple(res[::-1]))
        ### END YOUR SOLUTION
    ...
    def permute(self, new_axes):
        ...
        return NDArray.make(tuple(new_shape),tuple(new_strides),self.device,self._handle,self._offset)
        ### END YOUR SOLUTION
    ...
    def broadcast_to(self, new_shape):
        ...
        #new_array=NDArray.make(new_shape,self.strides,self.device,self._handle,self._offset)
        new_strides=list(self.strides)
        for i in range(len(self.shape)):
          assert (self.shape[i]==1) or (new_shape[i]==self.shape[i])
          if self.shape[i]==1 and new_shape[i]!=1:
            new_strides[i]=0
        new_array=NDArray.make(new_shape,tuple(new_strides),self.device,self._handle,self._offset)
    ...
    def __getitem__(self, idxs):
        ...
        ### BEGIN YOUR SOLUTION
        new_stride=[idxs[i].step*self.strides[i] for i in range(self.ndim)]
        new_shape=[1+(idxs[i].stop-idxs[i].start-1)//idxs[i].step for i in range(self.ndim)]
    ...
    def flip(self, axes):
        ...
        for i in range(self.ndim):
          if i==0:
            offset_change_eff.append(self.size-self.strides[0])
          else:
            offset_change_eff.append(self.strides[i-1]-self.strides[i])
        new_stride=list(self.strides)
        offset=self._offset
        for axis in axes:
          new_stride[axis]=-new_stride[axis]
          offset+=offset_change_eff[axis]
        return NDArray.make(self.shape,new_stride,self.device,self._handle,offset).compact()
        ### END YOUR SOLUTION
    ...
    def pad(self, axes):
        ...
        for i,axis in enumerate(axes):
          ...
          new_idx.append(slice(axis[0],axis[0]+self.shape[i]))
        pad_array=full(new_shape,0,dtype=self.dtype,device=self.device)
        pad_array[tuple(new_idx)]=self
        return pad_array
        ### END YOUR SOLUTION
