...
class MultiHeadAttention(Module):
    ...
    def forward(
        ...
        ### BEGIN YOUR SOLUTION
        qkt=self.matmul(q,k)/(q_dim**0.5)
        if self.causal:
          qkt+=Tensor(self.create_causal_mask(queries_len,keys_values_len,device=self.device),device=self.device).broadcast_to(qkt.shape)
        probs=self.dropout(self.softmax(qkt))
...
class AttentionLayer(Module):
    ...
    def forward(
        ...
        k=(self.k_projection(self.prenorm_k(k.reshape((batch_size*queries_len,k_dim)))))
        k=k.reshape((batch_size,keys_values_len,self.num_head,self.dim_head)).transpose((1,2))
        v=(self.v_projection(self.prenorm_v(v.reshape((batch_size*queries_len,v_dim)))))
        v=v.reshape((batch_size,keys_values_len,self.num_head,self.dim_head)).transpose((1,2))
        
        
        x,probs=self.attn.forward(q,k,v)
        x=x.transpose((1,2)).reshape((batch_size*queries_len,self.dim_head*self.num_head))
...
class TransformerLayer(Module):
    ...
    def __init__(
        ...
        *,
        ...
        self.attention=AttentionLayer(q_features,num_head,dim_head,dropout=dropout,device=device,dtype=dtype,causal=causal)
        self.dropout=Dropout(dropout)
    ...
    def forward(
        ...
        x=x+self.dropout3(self.linear2(self.dropout2(self.relu(self.linear1(self.norm(x))))))
        ### END YOUR SOLUTION
        x=x.reshape((batch_size,seq_len,x_dim))
        return x
...
class Transformer(Module):
    ...
    def __init__(
        ...
        *,
        ...
        self.transformers=[TransformerLayer(embedding_size,num_head,dim_head,hidden_size,dropout=dropout,causal=causal,device=device,dtype=dtype) for _ in range(num_layers)]
        ### END YOUR SOLUTION
    ...
    def forward(
        ...
        pos=Tensor(list(range(seq_len))).reshape((seq_len,1)).broadcast_to((seq_len,batch_size))
        pos=self.pos_embedding(pos).transpose((0,1))
        x=x+pos
